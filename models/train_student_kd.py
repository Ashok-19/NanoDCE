...
from loss_functions import Myloss as Myloss
import numpy as np
...
def train_single_config(config, alpha, beta, run_name):
    ...
    teacher_extractor = Feature_extractor.FeatureExtractor(teacher_net, is_student=False)
    ...
    student_extractor = Feature_extractor.FeatureExtractor(student_net, is_student=True)
    student_extractor.register_hooks()
    
    # Setup feature extraction for EMA model
    ema_extractor = Feature_extractor.FeatureExtractor(ema_student, is_student=True)
    ...
    if config.stage1_epochs > 0:
        ...
        for epoch in range(config.stage1_epochs):
            ...
            for iteration, img_lowlight in enumerate(train_loader):
                ...
                loss_multi_exposure = 7.0 * multi_exposure_loss_fn(student_enhanced_image, img_lowlight)
